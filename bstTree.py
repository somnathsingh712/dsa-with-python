...
def delete_user(root, target_id):
    ...
    if target_id < root.id:
        ...
        root.user_data = successor.user_data

        root.right = delete_user(root.right, successor.id)
...
def validate_bst_helper(node, min_val=-float("inf"), max_val=float("inf")):
    ...
    return (
        validate_bst_helper(node.left, min_val, node.id)
        and
        validate_bst_helper(node.right, node.id, max_val)
    )
